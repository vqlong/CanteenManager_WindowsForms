...
BEGIN TRANSACTION;
CREATE TABLE Account (Username NVARCHAR (100) PRIMARY KEY, DisplayName NVARCHAR (100) NOT NULL DEFAULT 'CafeNo1', Password NVARCHAR (1000) NOT NULL DEFAULT '952362351022552001115621782120108109105108121194219194572217814518010341215583925187233', Type INT NOT NULL DEFAULT 0);
...
CREATE TABLE FoodCategory (Id INTEGER PRIMARY KEY AUTOINCREMENT, Name NVARCHAR (100) NOT NULL DEFAULT 'Chưa đặt tên', CategoryStatus INT NOT NULL DEFAULT 1);
...
CREATE TABLE TableFood (Id INTEGER PRIMARY KEY AUTOINCREMENT, Name NVARCHAR (100) NOT NULL DEFAULT 'Chưa đặt tên', TableStatus NVARCHAR (100) NOT NULL DEFAULT 'Trống', UsingState INT NOT NULL DEFAULT 1);
...
CREATE TABLE Food (Id INTEGER PRIMARY KEY AUTOINCREMENT, Name NVARCHAR (100) NOT NULL DEFAULT 'Chưa đặt tên', CategoryId INT NOT NULL, Price DOUBLE NOT NULL DEFAULT 0, FoodStatus INT NOT NULL DEFAULT 1, FOREIGN KEY (CategoryId) REFERENCES FoodCategory (Id));
...
INSERT INTO sqlite_sequence VALUES('Bill',61);
INSERT INTO sqlite_sequence VALUES('BillInfo',98);
INSERT INTO sqlite_sequence VALUES('Food',32);
CREATE TRIGGER UTG_InsertBill AFTER INSERT ON Bill BEGIN UPDATE TableFood SET TableStatus = 'Có người' WHERE ID = new.TableID AND new.BillStatus = 0; END;
CREATE TRIGGER UTG_DeleteBill AFTER DELETE ON Bill BEGIN UPDATE TableFood SET TableStatus = 'Trống' WHERE ID = old.TableID AND old.BillStatus = 0; END;
CREATE TRIGGER UTG_UpdateBill AFTER UPDATE ON Bill BEGIN UPDATE TableFood SET TableStatus = 'Trống' WHERE ID = old.TableID AND ID = new.TableID AND old.BillStatus = 0 AND new.BillStatus = 1; UPDATE TableFood SET TableStatus = 'Có người' WHERE ID = new.TableID AND old.TableID != new.TableID AND old.BillStatus = 0 AND new.BillStatus = 0; UPDATE TableFood SET TableStatus = 'Trống' WHERE ID = old.TableID AND (SELECT count(*) FROM Bill WHERE TableID = old.TableID AND BillStatus = 0) = 0; END;
COMMIT;
