...
namespace CanteenManager
{
    ...
    public partial class fLogin : Form
    {
        ...
        /// <returns></returns>
        Account Login(string username, string password) => AccountDAO.Instance.Login(username, password);
        ...
        bool TestConnection()
        {
            ...
            string connectionStr = Config.ConnectionString;

            //Nếu mở panel AdvanceOption ra
            if (plAdvanceOption.Visible == true)
            {
                connectionStr = $@"Data Source=.\{txbServerName.Text};Initial Catalog={txbDatabase.Text};Integrated Security=True";
            }

            if (Config.DataProvider.TestConnection(connectionStr))
            {
        ...
        private void lblAdvanceOption_Click(object sender, EventArgs e)
        {
            ...
            if (!Config.DataProvider.Name.Equals("ADO.NET-SqlServer")) 
	        {
		        MessageBox.Show("Chức năng này chỉ hỗ trợ ADO.NET-SqlServer!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
		        return;
	        }	
